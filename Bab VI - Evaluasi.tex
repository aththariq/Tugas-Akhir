...
dinilai adalah apakah artefak realisasi yang tersedia benar-benar
...
\begin{enumerate}
  ...
  bahwa model data yang dirancang benar-benar digunakan sebagai wadah integrasi
...
pernyataan bahwa rancangan berhasil, tetapi menunjukkan dasar pemeriksaannya.
...
Keempat, skenario dinyatakan terpenuhi apabila bukti yang diperiksa menunjukkan
bahwa keputusan arsitektural tersebut benar-benar muncul pada perilaku sistem
yang direalisasikan.
...
\begin{table}[ht]
    ...
    \begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.18\textwidth} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}p{0.09\textwidth}}
        \toprule
...
\begin{table}[ht]
    ...
    \begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.18\textwidth} >{\raggedright\arraybackslash}p{0.18\textwidth} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}p{0.18\textwidth}}
        ...
        Terpenuhi pada realisasi sistem \\
        ...
        Terpenuhi untuk integrasi internal; siap dikembangkan untuk integrasi eksternal \\
        ...
        Interoperabilitas dan kemudahan pemeliharaan &
        NFR-02, NFR-05 &
        Pemisahan komponen \textit{edge}, API, web, basis data, dan penyimpanan objek menunjukkan batas tanggung jawab yang jelas serta kontrak layanan yang dapat dikembangkan bertahap. &
        Terpenuhi sebagai rancangan arsitektur \\
        \bottomrule
    \end{tabularx}
\end{table}
...
daftar fungsi yang berjalan, tetapi sebagai validasi bahwa rancangan arsitektur
memang implementabel dan mendukung tujuan yang telah ditetapkan.
...
\begin{enumerate}
  ...
  keputusan tersebut benar-benar dapat direalisasikan pada sistem yang dibangun.
\end{enumerate}
...
Dengan demikian, jalur video langsung terbukti mendukung kebutuhan operasional
tanpa mengganggu jalur penyimpanan data transaksional.
...
\begin{enumerate}
  \item Integrasi antarlayanan berhasil direalisasikan pada jalur yang memang
  ...
  pada tingkat diagram, tetapi benar-benar diwujudkan dalam perilaku sistem.
  ...
  Dengan demikian, struktur data yang dirancang berhasil mendukung agregasi
  ...
  Gambar~\ref{fig:evidence_ocr_live} dan Gambar~\ref{fig:evidence_defect_live},
  yang memperlihatkan bahwa aliran OCR dan \textit{defect scan} benar-benar
...
memperkuat kesimpulan bahwa arsitektur yang dibangun benar-benar berpusat pada
...
\begin{table}[ht]
    ...
    \begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X}
        ...
        \midrule
        Data inspeksi berasal dari beberapa proses dan harus tetap terhubung pada objek yang sama & Nomor kontainer dijadikan jangkar integrasi dan pusat agregasi data & Alur OCR membentuk atau memperbarui entitas kontainer, lalu hasil pemindaian lain dikaitkan ke entitas yang sama & Rancangan arsitektur data terbukti mendukung konsistensi konteks inspeksi \\
        Video langsung dan data transaksional memiliki karakteristik beban dan tujuan yang berbeda & Jalur video dipisahkan dari jalur data terstruktur & Video beranotasi disiarkan melalui \textit{WebSocket}, sedangkan hasil inspeksi dikirim ke layanan API & Pemisahan jalur komunikasi terbukti layak dan sesuai fungsi \\
        Hasil inspeksi harus dapat ditelusuri, diaudit, dan dibandingkan dengan konteks manifes & Data inti disimpan pada model kontainer dan manifes, sedangkan artefak visual dipisahkan ke penyimpanan objek & Koleksi \textit{containers} dan \textit{manifests} menyimpan data terstruktur, sedangkan bukti visual disimpan pada Cloudflare R2 & Rancangan penyimpanan terbukti mendukung ketertelusuran dan pengelolaan bukti inspeksi \\

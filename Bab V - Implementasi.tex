...
\newcommand{\implementationitem}[1]{%
...
Bab ini menjelaskan realisasi rancangan arsitektur sistem otomasi inspeksi kontainer
...
\begin{table}[ht]
  ...
  \begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{0.24\textwidth} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X}
    ...
    Infrastruktur operasional multi-lokasi dengan jaminan ketersediaan dan pengukuran performa skala pelabuhan. \\
    ...
    Evaluasi &
    Validasi skenario arsitektur: komunikasi antarkomponen, konsistensi data, pemisahan jalur, dan riwayat bukti &
    Evaluasi bisnis, evaluasi penerapan operasional, dan evaluasi kuantitatif terhadap performa model AI. \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{center}
  ...
  \centering
  \captionof{table}{Pemetaan Rancangan Arsitektur terhadap Realisasi Sistem}
...
\textit{frame} yang sudah diberi anotasi beserta \textit{metadata} deteksi yang diperlukan
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    font=\small,
    block/.style={draw, rounded corners, align=center, minimum width=2.70cm, minimum height=0.92cm},
    data/.style={draw, align=center, minimum width=2.70cm, minimum height=0.92cm},
    arrow/.style={-{Latex[length=2mm]}, thick}
  ]
    \node[block] (camera) at (0,2.2) {Kamera\\\textit{RTSP}};
    \node[block] (tepi) at (3.25,2.2) {Komponen\\\textit{Edge}};
    \node[block] (api) at (6.50,2.2) {Layanan API\\\texttt{/api/...}};
    \node[data] (db) at (9.75,2.2) {MongoDB\\\textit{containers}};
    \node[block] (web) at (3.25,-0.42) {Aplikasi Web};
    \node[data] (r2) at (8.00,-0.42) {Cloudflare R2\\Artefak Visual};

    \draw[arrow] (camera) -- (tepi);
    \draw[arrow] (tepi) -- (api);
    \draw[arrow] (api) -- (db);
    \draw[arrow] (api.south east) -- node[right]{simpan artefak} (r2.north);
    \draw[arrow] (web.north east) -- node[left]{REST} (api.south west);
    \draw[arrow] (web) -- node[below]{akses URL} (r2);
    \draw[arrow] (tepi.south) -- node[left]{\textit{WebSocket}} (web.north);
  \end{tikzpicture}}
...
Salah satu kebutuhan integrasi utama dalam sistem ini adalah propagasi nomor
kontainer, yaitu mekanisme untuk menyelaraskan hasil identifikasi kontainer
dengan aliran pemeriksaan lainnya. Pada realisasinya, ketika aliran OCR
berhasil mendeteksi nomor kontainer, nomor tersebut secara otomatis
dipropagasikan ke aliran lain yang sedang aktif.
...
Dengan demikian, realisasi sistem menunjukkan bahwa kebutuhan integrasi data
lintas modul telah diwujudkan pada kapabilitas arsitektur inti yang diuji.
...
\implementationitem{Entitas \textit{Container}}
...
\implementationitem{Entitas \textit{Manifest}}
...
\implementationitem{Konteks autentikasi dan akses pengguna}
...
\implementationitem{Pemisahan penyimpanan terstruktur dan artefak visual}
...
Keputusan ini menjawab kebutuhan arsitektural yang telah dirumuskan pada
...
menjaga struktur data utama tetap ringkas dan mengurangi kebutuhan penyimpanan
media berukuran besar di basis data transaksional.

\implementationitem{Dukungan terhadap riwayat kontainer dan audit}
Salah satu tujuan penting dari rancangan arsitektur adalah menyediakan
riwayat proses inspeksi pada tingkat kontainer. Pada implementasinya, tujuan tersebut
...
Pada ruang lingkup kontribusi tugas akhir, artefak realisasi ini menunjukkan bahwa
...
bagi sistem yang direalisasikan pada proyek pengembangan sistem. Hal tersebut
menunjukkan bahwa kontribusi tugas akhir ini terletak pada rancangan arsitektur
yang memiliki padanan implementasi, kontrak integrasi, dan dukungan terhadap
riwayat proses inspeksi di lingkungan pelabuhan. Dengan kata lain, nilai
utama bab ini bukan pada rincian kode, melainkan pada keterhubungan antara
rancangan arsitektural dan perilaku sistem yang direalisasikan.
